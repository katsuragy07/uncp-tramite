...
$query_rs1 = sprintf("
SELECT folioint.*
,(SELECT CONCAT(empleado.apellido,', ',empleado.nombre) FROM empleado WHERE folioint.empid=empleado.id)AS enombre
,(SELECT CONCAT(lugares.nombre,' | ',oficinas.nombre) FROM oficinas,lugares,folioint WHERE oficinas.id=folioint.c_oficina AND oficinas.lugares_id=lugares.id LIMIT 1)AS onombre
FROM folioint 
WHERE folioint.id=%s", GetSQLValueString($colname_rs1, "int"));
...
mysql_select_db($database_cn1, $cn1);
$query_rs2 = "SELECT log_archivo_int.*
,(SELECT CONCAT(empleado.apellido,', ',empleado.nombre) FROM empleado WHERE log_archivo_int.empid=empleado.id)AS enombre
,(SELECT CONCAT(lugares.nombre,' | ',oficinas.nombre) FROM oficinas,lugares,empleado 
WHERE empleado.id=log_archivo_int.empid AND oficinas.lugares_id=lugares.id AND oficinas.id = empleado.oficinas_id
LIMIT 1)AS onombre
FROM log_archivo_int
WHERE log_archivo_int.folioint_id=$colname_rs2 
AND log_archivo_int.tipo=1
ORDER BY log_archivo_int.fecha ASC";
...
mysql_select_db($database_cn1, $cn1);
$query_rs3 = sprintf("SELECT log_derivar_int.*
,(SELECT CONCAT(empleado.apellido,', ',empleado.nombre) FROM empleado WHERE log_derivar_int.empid=empleado.id)AS enombre
,(SELECT CONCAT(lugares.nombre,' | ',oficinas.nombre) FROM oficinas,lugares,folioext WHERE oficinas.id=log_derivar_int.d_oficina AND oficinas.lugares_id=lugares.id LIMIT 1)AS onombre
FROM log_derivar_int
WHERE log_derivar_int.folioint_id=%s
AND log_derivar_int.tipo=1
ORDER BY log_derivar_int.fecha ASC", GetSQLValueString($colname_rs3, "int"));
...
<script type="text/javascript">function dlitem(ord){if(confirm("Deseas eliminar este registro?")){document.location.href= '../opers/del_tdin_nuevo.php?pk='+ord;}}</script> 
<script type="text/javascript">function dlitem2(ord){if(confirm("Deseas eliminar este registro?")){document.location.href= '../opers/del_tdinoper.php?pr='+ord+'&pk=<?php echo $_GET['pk'];?>';}}</script> 

</head>
<body>
<div id="container"><div id="wpag"><div id="content">

<h1>Expediente Interno &raquo; <?php echo $row_rs1['firma']; ?></h1>
...
<td width="40%" valign="top"><h4>Datos de creación</h4>
    <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="tabla">
      ...
      <a href="../data/tdin_adjuntos/<?php echo $row_rs1['file'];?>" class="left" target="_blank" title="Descargar el archivo adjunto"><img src="../images/<?php echo dtarchivo($row_rs1['ext']);?>" style="border:none;" /></a><div class="left" style="margin:5px 0 0 15px;" >Tipo: <?php echo strtoupper($row_rs1['ext']); ?><br>Tamaño: <?php echo $row_rs1['size']; ?></div>
 <?php }else{ ?>No<?php } ?></td>
        </tr>
      </table></td>
</tr>
      ...
      <a href="../data/tdin_adjuntos/<?php echo $row_rs3['file'];?>" target="_blank" title="Descargar el archivo adjunto"><img src="../images/<?php echo dtarchivo($row_rs3['ext']);?>" style="border:none;" /></a>
      <div class="min" align="center"><?php echo $row_rs3['size'];?></div>
      <?php }else{ ?>No<?php } ?></td>
              <?php if(dribbut($vrs1,$vrs3)){?>
              <td width="80">
                 <div class="spacer"><a href="../opers/mod_tdinoper.php?pk=<?php echo $_GET['pk']; ?>&fk=<?php echo $row_rs3['id']; ?>"><span class="skin left" style="background-position:-48px -63px;margin-right:3px;"></span> Editar</a></div>
      ...
      <a href="../data/tdin_adjuntos/<?php echo $row_rs2['file'];?>" target="_blank" title="Descargar el archivo adjunto"><img src="../images/<?php echo dtarchivo($row_rs2['ext']);?>" style="border:none;" /></a><div class="min" align="center"><?php echo $row_rs2['size'];?></div><?php }else{ ?>No<?php } ?></td>
              <?php if(dribbut($vrs1,$vrs2)){?>
              <?php } ?>
            </tr>
            <?php } // Show if recordset not empty ?>
...
<?php if(dribbut($vrs1,$vrs3)){?>
<div class="spacer"><a href="../opers/mod_tdin_nuevo.php?pk=<?php echo $_GET['pk']; ?>" >
...
<?php } ?>
<div class="spacer"><a href="../reportes/tdin_verfolio.php?pk=<?php echo $_GET['pk']; ?>" target="_blank"><div class="skin left" style="background-position:-0px -79px;margin-right:3px;"></div>Preparar para impresi&oacute;n</a></div>
