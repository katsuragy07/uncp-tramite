...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO users (id, nombre, pass, empleado_id, `level`, perfil_id, snom) VALUES (%s, %s, %s, %s, %s, %s, %s)",
                       GetSQLValueString($_POST['id'], "text"),
                       GetSQLValueString($_POST['nombre'], "text"),
                       GetSQLValueString($_POST['pass'], "text"),
                       GetSQLValueString($_POST['empleado_id'], "int"),
...
mysql_select_db($database_cn1, $cn1);
$query_rs4 = sprintf("SELECT nombre, apellido, encargado FROM empleado WHERE id = %s", GetSQLValueString($colname_rs4, "int"));
...
function gpass() {
	var strCaracteresPermitidos = 'a,b,c,d,e,f,g,h,j,k,m,n,p,q,r,s,t,u,v,w,x,y,z,1,2,3,4,5,6,7,8,9';
    ...
    <form method="post" name="form1" action="<?php echo $editFormAction; ?>">
      <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="tabla">
        ...
        <tr valign="baseline">
          ...
          <input name="nombre" type="text" class="tbox2" id="nombre" value="<?php echo dnuser($row_rs4['encargado'],$row_rs4['apellido'],$row_rs4['nombre']); ?>" /><span class="textfieldMinCharsMsg"><br/>Se necesitan 4 car&aacute;cteres como m&iacute;nimo</span></span></td>
          </tr>
        <tr valign="baseline">
          <td width="150" align="right" valign="middle">Contraseña</td>
          <td><span id="sprytextfield2">
            <input name="pass" type="text" class="tbox1" id="pass" value="" />
...
<input type="button" onclick="gpass();" name="button" id="button" value="Generar" /></td>
        ...
        <tr valign="baseline">
          ...
          <td>
            <select name="level" id="level" onchange="selca();">
              <option value="1" selected="selected">Personal Administrativo</option>
        ...
        <tr valign="baseline" name="hos1" id="hos1">
          ...
          <td valign="middle"><select name="snom2" size="2" multiple="multiple" id="snom2" onchange="camb();">
              <?php
do {  
?><option value="<?php echo $row_rs2['id']?>"<?php if (!(strcmp($row_rs2['id'], 2))) {echo "selected=\"selected\"";} ?>><?php echo $row_rs2['nombre']?></option>
              <?php
...
?>
      ...
      <input type="hidden" name="id" value="<?php echo "us".did();?>">
      <input type="hidden" name="snom" value="1">
      <input type="hidden" name="empleado_id" value="<?php echo $_GET['pk']?>">
