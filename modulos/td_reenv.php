...
FROM log_derivar
WHERE log_derivar.c_oficina = $varssess
...
function suggest(inputString,inputOfice){console.log(inputOfice);if(inputString.length==0){$('#suggestions').fadeOut()}else{$('#pname').addClass('load');$.post("../includes/autos_exp_derivados.php",{queryString:""+inputString+"",queryOfice:""+inputOfice+""},function(data){if(data.length>0){$('#suggestions').fadeIn();$('#suggestionsList').html(data);$('#pname').removeClass('load')}})}}function fill(thisValue){$('#pname').val(thisValue);setTimeout("$('#suggestions').fadeOut();",600)}var default_content="";
</script>
</head>
<body>
...
<h1>Expedientes Derivados (Externos)</h1>
    ...
    <h2>Lista de expedientes derivados desde la oficina</h2>
        <table width="90%" border="0" cellpadding="0" cellspacing="0" class="tabla2">
          <tr>
            ...
            <td align="center" class="btit_1">N° de<br />
              Folios</td>
