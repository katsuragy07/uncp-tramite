...
$query_rs1 = "SELECT log_derivar_int.*
,(SELECT CONCAT(empleado.apellido,', ',empleado.nombre) FROM empleado WHERE empleado.id = log_derivar_int.empid LIMIT 1) AS enombre
,(SELECT CONCAT(lugares.nombre,' | ',oficinas.nombre) FROM oficinas,lugares,log_derivar_int WHERE oficinas.id=log_derivar_int.c_oficina AND oficinas.lugares_id=lugares.id LIMIT 1)AS onombre
...
WHERE log_derivar_int.c_oficina = $varssess
...
function suggest(inputString,inputOfice){console.log(inputOfice);if(inputString.length==0){$('#suggestions').fadeOut()}else{$('#pname').addClass('load');$.post("../includes/autos_expin_derivados.php",{queryString:""+inputString+"",queryOfice:""+inputOfice+""},function(data){if(data.length>0){$('#suggestions').fadeIn();$('#suggestionsList').html(data);$('#pname').removeClass('load')}})}}function fill(thisValue){$('#pname').val(thisValue);setTimeout("$('#suggestions').fadeOut();",600)}var default_content="";
</script>
</head>
<body>
...
<h1>Expedientes Derivados (Internos)</h1>
...
<table <?php echo dtabla();?> border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <h2>Lista de expedientes derivados desde la oficina</h2>
